...
void writeFastVLine(int16_t x, int16_t y,
        int16_t h,Colours8 Col) {
    // Overwrite in subclasses if startWrite is defined!
    // Can be just writeLine(x, y, x, y+h-1, color);
    // or writeFillRect(x, y, 1, h, color);
    drawFastVLine(x, y, h, Col);
}
...
uint8_t FillCircle(uint8_t XS, uint8_t YS, uint8_t R, Colours8 LCol, Colours8 FCol){
	return FillEllipse(XS, YS, R, R, LCol, FCol);
}
...
uint8_t FillEllipse(uint8_t XS, uint8_t YS, uint8_t XR, uint8_t YR, Colours8 LCol, Colours8 FCol){
	//uint8_t XCnt, YCnt, LCnt;

	uint8_t FRowLen = IAbs(XR-XS)-1, FColLen = IAbs(YR-YS)-1;
	uint32_t Cnt, Area = FRowLen*FColLen;

	LineC(XS, YS, XR, YS, LCol);
	LineC(XS, YR, XR, YR, LCol);
	LineC(XS, YS, XS, YR, LCol);
	LineC(XR, YS, XR, YR, LCol);

	SetAddr(XS+1, YS+1, XR-1, YR-1);
...
void fillCircle(int16_t x0, int16_t y0, int16_t r,
		 Colours8 Col) {
    startWrite();
    writeFastVLine(x0, y0-r, 2*r+1,Col);
    fillCircleHelper(x0, y0, r, 3, 0, Col);
    endWrite();
}

void fillCircleHelper(int16_t x0, int16_t y0, int16_t r,
        uint8_t cornername, int16_t delta,  Colours8 Col) {
    ...
    while (x<y) {
        ...
        if (cornername & 0x1) {
            writeFastVLine(x0+x, y0-y, 2*y+1+delta, Col);
            writeFastVLine(x0+y, y0-x, 2*x+1+delta, Col);
        }
        if (cornername & 0x2) {
            writeFastVLine(x0-x, y0-y, 2*y+1+delta, Col);
            writeFastVLine(x0-y, y0-x, 2*x+1+delta, Col);
...
uint8_t Triangle(uint8_t X, uint8_t Y, uint8_t SideLen, uint16_t Angle, Colours8 Col){
	int32_t TCoX, TCoY, LCoX, LCoY, RCoX, RCoY;
	int32_t NTCoX, NTCoY, NLCoX, NLCoY, NRCoX, NRCoY;
	uint16_t SLST = SideLen>>1, HST, Height;
